...
public class PushTimerRequest implements Request {

	////////// REQUEST ////////////

	public PushTimerRequest() {
		new RequestList().addRequest(this, getType());
	}

	private static Request request = new PushTimerRequest();
	...
	public void doAction(PiranhaObject object, String target) {
		...
		DIRECTION facing = getFacing(targets[0]);
		int pushingTime = Integer.parseInt(targets[1]);

		if (facing == null || facing == DIRECTION.NULL)
			return;

		Pusher pusher = getPusher(object, facing, pushingTime);
	...
	private Pusher getPusher(PiranhaObject object, DIRECTION facing, int pushingTime) {
		...
		Pusher pusher = new Pusher() {
			...
			public int getPushingTime() {
				...
				return fastMode.getState() ? 3 * pushingTime : pushingTime;
			}
