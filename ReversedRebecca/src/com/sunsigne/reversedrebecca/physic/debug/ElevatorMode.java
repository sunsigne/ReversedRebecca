...
public class ElevatorMode extends DebugMode {
	...
	private void upPlayer() {
		putPlayerFromAtoB(LAYER.WORLD_CONTENT.getHandler(), LAYER.UP_WORLD_CONTENT.getHandler());
		putPlayerFromAtoB(LAYER.DOWN_WORLD_CONTENT.getHandler(), LAYER.WORLD_CONTENT.getHandler());
	}

	private void downPlayer() {
		putPlayerFromAtoB(LAYER.WORLD_CONTENT.getHandler(), LAYER.DOWN_WORLD_CONTENT.getHandler());
		putPlayerFromAtoB(LAYER.UP_WORLD_CONTENT.getHandler(), LAYER.WORLD_CONTENT.getHandler());
	}

	private void putPlayerFromAtoB(Handler handlerA, Handler handlerB) {
		Player player = new PlayerFinder().getPlayer();
